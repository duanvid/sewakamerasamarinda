...
export default function CreatePost() {
  ...
  return (
    <div className="grow flex flex-col items-center justify-center p-5 w-full ">
      <h2 className="p-5 text-lg">Create Blog Post</h2>
      <form action="submit" className=" bg-white h-full flex flex-col border p-5 rounded-md shadow-md w-full overflow-y-auto" onSubmit={onSubmitHandle}>
        <label htmlFor="blogitle" className="p-2 text-slate-800">Title</label>
        <input type="text" id="blogtitle" className="border-b p-2 rounded-md" placeholder="Blog Title" required ref={blogTitle}/>
        <label htmlFor="blogcontent" className="p-2 text-slate-800">Content</label>
        <ReactQuill className="flex flex-col grow" theme="snow" ref={blogContent} id="blogcontent" modules={modules}/>
        <label htmlFor="blogimg" className="p-2">Image</label>
        <input type="file" name="blogimg" id="blogimg" className="p-2 border w-fit" />
